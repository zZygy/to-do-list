...
export const Tasks = ({ taskList, setTaskList }: { taskList: string[], setTaskList: React.Dispatch<React.SetStateAction<string[]>> }) => {
  ...
  const saveNewTaskName = (index: number) => {
    ...
    updateTaskList.splice(index, 1, taskFormatted);
    ...
    <div>
      {taskList.map((task, index) => (
        <div key={index} className="flex justify-between">
          ...
          {editIndex !== index  &&
            <button onClick={() => editTask(index, task)}>
